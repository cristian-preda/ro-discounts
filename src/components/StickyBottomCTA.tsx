...
export function StickyBottomCTA({ telegramLink }: StickyBottomCTAProps) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Show sticky CTA after scrolling past hero (roughly 300px for better mobile UX)
  ...
  return (
    ...
    >
      <a href={telegramLink} target="_blank" rel="noopener noreferrer" className="block">
        <Button
          size="lg"
          className="bg-[#5B8CFF] hover:bg-[#4a7ae6] text-white text-lg font-bold rounded-full px-8 py-6 shadow-2xl hover:shadow-[#5B8CFF]/30 transition-all w-full"
        >
          Intră pe Telegram →
        </Button>
      </a>
